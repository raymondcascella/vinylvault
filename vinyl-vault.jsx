...
export default function VinylVault() {
  ...
  async function dig(q = query, branchSong = null) {
    const term = (branchSong ? `${branchSong.title} by ${branchSong.artist}` : q).trim();
    if (!term) return;
    setLoading(true);
    setError("");
    setLastQuery(branchSong ? `${branchSong.title} — ${branchSong.artist}` : term);

    const depthGuidance = [
      "the most famous, chart-topping, universally known songs",
      "mostly popular songs but slip in 2-3 lesser-known fan favorites",
      "a deliberate mix: roughly one-third recognizable, two-thirds lesser-known album tracks and overlooked songs",
      "deep album cuts, B-sides, and tracks that only dedicated fans would know — avoid the obvious singles",
      "the most obscure, buried, hard-to-find tracks: rarities, demos, deep B-sides, the stuff that makes diggers feel clever",
    ][depth];

    // Size the request to the fill target: ~4 min per track, clamped to a sane range.
    const trackCount = Math.max(5, Math.min(30, Math.round(targetMin / 4)));

    let prompt;
    if (branchSong) {
      prompt = `You are a record-store clerk with encyclopedic taste. A patron just heard "${branchSong.title}" by ${branchSong.artist} (${branchSong.year}) and said "give me more like THIS one."

Suggest exactly ${trackCount} real, existing songs that share this track's SONIC character — its feel, mood, tempo, instrumentation, production style, and era — songs that would sit beautifully next to it on a mixtape. Weight the picks toward ${depthGuidance}.

Rules:
- Only real songs that actually exist. Never invent titles.
- Match the VIBE, not just the genre label — chase the actual sound and feeling of "${branchSong.title}".
- Include AT MOST ONE other song by ${branchSong.artist}; the point is to discover kindred tracks by OTHER artists. Do NOT include "${branchSong.title}" itself.
- Favor variety: NO MORE THAN 2 songs by any single artist across the ${trackCount} picks; spread across many different artists.
- "obscurity" is 0-100 where 0 = ubiquitous smash hit, 100 = almost nobody knows it.
- "note" is a short, vivid one-liner (max ~12 words) on what it shares with "${branchSong.title}".
- "duration" is the track's runtime in SECONDS (a realistic estimate of the actual recording's length).

Respond with ONLY a JSON array, no markdown, no preamble. Each object:
{"title": string, "artist": string, "year": number, "obscurity": number, "note": string, "duration": number}`;
    } else {
      const includeSeed = Math.random() < 0.5;
      const seedRule = includeSeed
        ? `- If "${term}" names a specific artist or band, include EXACTLY ONE song by that exact artist, and fill the other ${trackCount - 1} with adjacent, similar, or influencing/influenced artists the patron likely hasn't explored.`
        : `- If "${term}" names a specific artist or band, include ZERO songs by that exact artist. Use them purely as a compass: every pick should be by adjacent, similar, or influencing/influenced artists the patron likely hasn't explored.`;

      prompt = `You are a record-store clerk with encyclopedic taste who loves turning people on to music they haven't heard. The patron asked about: "${term}" (this could be an artist, band, genre, era, or mood).

Treat "${term}" as a GUIDE or starting point, NOT a destination. The goal is discovery beyond it.

Suggest exactly ${trackCount} real, existing songs that fit, weighted toward ${depthGuidance}.

Rules:
- Only real songs that actually exist. Never invent titles.
${seedRule}
- If "${term}" is a genre/mood/era, draw from a wide SPREAD of different artists with no single act dominating.
- Favor variety: NO MORE THAN 2 songs by any single artist across the ${trackCount} picks; spread across many different artists.
- "obscurity" is 0-100 where 0 = ubiquitous smash hit, 100 = almost nobody knows it.
- "note" is a short, vivid one-liner (max ~12 words) on why this track is worth hearing.
- "duration" is the track's runtime in SECONDS (a realistic estimate of the actual recording's length).
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: Math.min(8000, 600 + trackCount * 130),
      ...
      parsed.sort((a, b) => a.obscurity - b.obscurity);
      // Hard cap: no more than MAX_PER_ARTIST songs from any single artist.
      const MAX_PER_ARTIST = 2;
      const seen = {};
      const capped = parsed.filter((s) => {
        const k = String(s.artist || "").trim().toLowerCase();
        seen[k] = (seen[k] || 0) + 1;
        return seen[k] <= MAX_PER_ARTIST;
      });
      setResults(capped.map((p, i) => ({ ...p, _id: `${Date.now()}-${i}` })));
      setTimeout(() => {
        listRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 80);
    } catch (e) {
      setError("The record's skipping — couldn't load selections. Drop another coin and try again.");
    } finally {
      setLoading(false);
    }
  }
            ...
            results.map((song, i) => {
              ...
              >
                ...
                <div style={S.rowMain}>
                  ...
                  <div style={S.rowMeta}>{song.artist} · {song.year}{song.duration ? ` · ${fmtTrack(song.duration)}` : ""}</div>
